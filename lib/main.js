...
function iterator( options ) {
	...
	if ( arguments.length > 0 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0q92V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'normalized' ) ) {
			if ( !isBoolean( opts.normalized ) ) {
				throw new TypeError( format( '0q92o,GE', 'normalized', opts.normalized ) );
			}
		}
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0q92t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
